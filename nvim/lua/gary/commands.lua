...
local t = {
    Build = { name = build_commands, keymap = "cb", desc = "Release build" },
...
for name, command_table in pairs(t) do
    nnoremap(
        ...
        "<Cmd>" .. name .. "<CR>",
        { silent = true, desc = "Command: " .. command_table["desc"] }
    )
...
nnoremap("<leader>cr", "<Cmd>Ha<CR>", { silent = true, desc = "Command: Build and Run" })
